...
const CourseEnrollUpdate = (props)  => {

    const [open, setOpen] = useState(false);
    const [editMessage, setEditMessage] = useState('');
    const [sections, setSections] = useState({courseId:'', title:'', credits:''});
    ...
    const editOpen = (event) => {
        setOpen(true);
        setEditMessage('');
        setSections(props.section);
    };

    const editClose = () => {
        setOpen(false);
        setSections({courseId:'', title:'', credits:''});
...
export default CourseEnrollUpdate;
